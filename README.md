...
Hoisting: ამდროს ჩვენი კოდის თარჯიმანს ცვალდები ფუნქციები და კლასების დეკლარაცია როგორც ჩანს აყავს მათი არეალის ზედა ნაწილში, კოდის შესრულებამდე.
  
რახდება ამის უკან?: ჩვენი კოდის შესრულებამდე კოდის სკანირედბა variable declaration_ისთვის და ყველა ცვლადისთვის იქმნება ახალი თვისება variable environment object_ში ეს ხდება creation phase_ში.

ეს ყველაფერი კარგად მოქმედებს მხოლოდ ფუნქციებზე დანარჩენი დაიკიდეთ.

TDZ(temporal dead zone): მოკელდ ეს არის ის ადგილი სადაც რომელიღაც ცვლადის გამოყენება არ შეგვიძლია, TDZ გვეხმარება ერორების უფრო მარტივად უფრო მარტივად აღმოჩენაში და ის const ცვლადს რეაულრად ამუშავებს.

2) Scope chain
   * Scoping: ეს ნიშნავს თუ როგორ არიან ჩვენი ცვლადები დაორგანიზებული და სად შეგვიძლია მივწვდეთ მათ
   * Lexical scoping: ამდროს ჩვენი სქოუფინგი მთლიანად კონტროლდება ფუნქციების და ბლოკ კოდების მიერ
   * Scope: ეს არის გარემო რაღაც სადაც ჩვენი ცვლადები არიან გამოცხადებულნი გვაქვს: global scope, function scope, block scope
   * Scope of a variable: ჩვენი კოდის რაღაც ნაწილი სადაც რომელიმე ცვლადი არის ხელმისაწვდომი

ცვლადები არარის ხელმისაწვდომი დაბლიდან მაღლა.
...
## The Call Stack
ქოლსთაქი არის ადგილი სადაც ერთმანეთზე ეწყობა execution context_ები რათა მათი შესრულების თანმიმდევვრობა არ აგვერიოს ის გავს რაღაცნაირად მაპს რომელიც გვიჩვენებს თუ რომელი კონტექსტი უნდა შესრულდეს ახლა, პირველი როგორც ვთქვით არის global context შემდგომ მას თავზე ედება ფუნქციები და ასშ... როდესაც ეს ფუნქციები დაასრულებენ შესრულებას ბოლოს მოდის ისევ გლობალ კონტექსტი და აქ უკვე ჩვენი კოდი რჩება შესრულებას პროგრამა იქამდე გააგრძელებს შესრულებას სანამ მთელი კოდი არ შესრულედება ან მომხმარებელი საიტს არ გამორთავს


## Primitives Vs Objects (Primitive types vs refrence types)
Primitives:
  * Number           
  * String               
  * Boolean                  
...
მოკლედ როგორც ვიცით ძრავს ორი მხარე აქვს ესენია callstack და heap, ქოლსთაქში სრულდება ჩვენი ფუნქციები ხოლო ჰიპი კი ინახავს მეხსიერებაში ყველა იმ ობიექტს რომელიც ჩვენს პროგრამას ჭირდება ასერომ refrence types ინახება ჰიპში ხოლო პრიმიტივები execution context_ში რომელიც თავის მხირვ არის ქოლსთაქში.

ესეიგი როდესაც ვქმნით ცვლადს რახდება ამდროს ჯავასკრიპტის ძრავში და კომპიუტერი მეხსიერებაში?

ესეიგი პირველად ჯავასკრიპტი ქმნის უნიკალურ იდენთიფიკატორს ცვლადის სახელით შემდეგ მისამართს მაგ 0001 ანუ ეს ჩვენი მნიშვნელობა ინახება ამ მისამართზე და საბოოლოდ მნიშვნელობა ანუ Identifier, Address, Value ესენი იქმნება.

და რათქმაუნდა ეს ყველაფერი ხდება ქოლსთაქში სადაც პრიმიტივები ინახებიან, ხოდა ეს ჩვენი იდენთიფიკატორი მიუთითებს იმ მისამართზე სადაც არის ეს ცვლადი და არა თვითონ მნიშვნელობაზე ანუ ეს ჩვენი იდენთიფიკატორი მიუთითებს 0001_ზე იმ მისამართზე რომელსაც უკავია ჩვენი მნიშვნელობა.

ახლა კი განვიხილოთ ასეთი შემთხვევა მაგალითად გვაქ ცვლადი age = 30 და ამის დაბლა გვაქ oldAge = age ამდროს ეს oldAge ცვლადი იღებს იმ მისამართს რაც აქვს age_ს და ეს მისამართი თავის მხრივ მიუთითებს 30_ზე ხოლო თუ ჩვენ age ცვლადს შევცვლით 31 oldAge არ შეიცვლება ეს იმიტომ რომ ჩვენ ცვლადის მისამართის მნიშნელობის შეცვლა არ შეგვიძლია ჩვენ მხოლოდ ვცვლით მისამართს და ჩვენი age ცვლადი გადავიყვანეთ 0002_ზე რომელსაც თავის მხრივ უჭირავს ეს 31 მნიშვნელობა და ასერომვთქვათ ახლა ეს იდენთიფიკატორი მიუთითებს ამ ახალ მისამართზე ხოლო oldAge დარჩა ისევ იგივეზე.

ესეიგი როდესაც იქმნება ახალი ობიექტი ის ინახება ჰიპში მაგრამ მისი იდენთიფიკატორი არა ჰიპში მხოლოდ გადადის ადრესი მაგ D30F და მნიშვნელობა რაცაა ხოლო ჩვენი იდენთიფიკატორი არ მიუთითებს ამ ახლად შექმნილ მისამართზე(D30F) ჰიპში არამედი ჩვენი იდენთიფიკატორი იქმნება callstack_ში იქმნება ახალი მისამართი აქვე მაგ 0003 და ამის მნიშვნელობა ხდება ჰიპში შექმნილი მისამართი ანუ D30F მარტივად რომ ვთქვათ ეს მეხსიერების ნაწილი რომელიც არის ქოლსთაქში მიუთითებს იმ მეხსიერების ნაწილზე რომელიც არის ჰიპში ამიტომაც ვეძახით მაც მიმთითებლებს, ობიექტები იმიტომ არ ინახება სთაქში რომ ისინი არიან ძალიან დიდები ამისთვის ამის მაგრივად ისინი ინახებიან ჰიპში რომელიც გავს განუსაზღვრავ მეხსიერების აუზსს ხოლო სთაქს მხოლოდ უჭირავს მიმთითებელი რომელცი მიუთითებს სად ინახება ობიექტი ჰიპში.

ხოლო თუ გვაქ მსგავსი შემთხვევა რომ მაგალითად გვაქ me object და გვაქ friend რომელიც უდრის me ობიექტს ამ შემთხვევაში თუ შევცვლით me ობიექტს მაშინ შეიცვლება friend_იც მოკლედ ავხსნათ ეხლა რატომ ხდება ასე როდესაც ჩვენ ჩვენს ობიექტში რამეს ვცვლით ეს იდენთიფიკატორი პოულობს ჩვენს ობიექტს ჰიპში და უცვლის მას მნიშვნელობას ანუ ჩვენ არ ვუცვლით მნიშვნელობას იდენთიფიკატორს ანუ ჩვენ არ ვცვლით D30F_ს ჩვენ ამ მნიშნველობით ვპოულობთ ობიექტს და მის მნიშნვლობას ვცვლით და ეს არარის პრობლემა ასე რომ ვთქვათ და ასევე ჩვენ შეგვიძლია გამოვიყენოთ const ობიექტების დროს და ის უპრობლემოდ შევცვალოთ, ასერომ ჩვენი me ობიექტი ისევ იგივე მისამართზეა რადგან ჩვენ არ შეგვიცვლია მისამართი ქოლსთაქში.
